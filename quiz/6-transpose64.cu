...
{
    int left = blockIdx.x * K;
    int top = blockIdx.y * K;

    __shared__ float s[K*K];

    s[threadIdx.x * K + threadIdx.y] = in[(top + threadIdx.y) * N + left + threadIdx.x];
    ...
    out[(top2 + threadIdx.y) * N + left2 + threadIdx.x] = s[threadIdx.y * K + threadIdx.x];
}
...
__global__ void 
transpose_parallel_per_element_tiled2(float in[], float out[])
{
    int left = blockIdx.x * 2 * K;
    int top = blockIdx.y * 2 * K;

    __shared__ float s[2*K*2*K];

	int i = 4*(threadIdx.x * K + threadIdx.y);
	int row = i / (2*K);
	int col = i % (2*K);

	s[col * 2 * K + row] = in[(top + row) * N + left + col];
	s[(col+1) * 2 * K + row] = in[(top + row) * N + left + col + 1];
	s[(col+2) * 2 * K + row] = in[(top + row) * N + left + col + 2];
	s[(col+3) * 2 * K + row] = in[(top + row) * N + left + col + 3];
	...
	out[(top2 + row) * N + left2 + col] = s[row * 2 * K + col];
	out[(top2 + row) * N + left2 + col + 1] = s[row * 2 * K + col + 1];
	out[(top2 + row) * N + left2 + col + 2] = s[row * 2 * K + col + 2];
	out[(top2 + row) * N + left2 + col + 3] = s[row * 2 * K + col + 3];
}
 ...
 */
	...
	dim3 blocks2(N/K/2,N/K/2); // blocks per grid
	dim3 threads2(K,K);	// threads per block
	...
	timer.Start();
	transpose_parallel_per_element_tiled2<<<blocks2,threads2>>>(d_in, d_out);
	timer.Stop();
	cudaMemcpy(out, d_out, numbytes, cudaMemcpyDeviceToHost);
	printf("transpose_parallel_per_element_tiled2 %dx%d: %g ms.\nVerifying ...%s\n", 
		   2*K, 2*K, timer.Elapsed(), compare_matrices(out, gold) ? "Failed" : "Success");
