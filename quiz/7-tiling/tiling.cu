...
__global__ void foo(float out[], float A[], float B[], float C[], float D[], float E[]){
	...
	__shared__ float sout[BLOCKSIZE];
	...
	sD[threadIdx.x] = D[i];
	sE[threadIdx.x] = E[i];
	sout[threadIdx.x] = (sA[threadIdx.x] + sB[threadIdx.x] + sC[threadIdx.x] + sD[threadIdx.x] + sE[threadIdx.x]) / 5.0f;

	out[i] = sout[threadIdx.x];
}
...
__global__ void bar(float out[], float in[]) 
{
	...
	__shared__ float s[BLOCKSIZE+4];
	...
	out[i] = (s[t] + s[t+1] + s[t+2] + s[t+3] + s[t+4]) / 5.0f;
}
